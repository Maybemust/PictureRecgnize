...
void AvoidCollision::start()
{
	...
	{ 
		...
		if(rightdown_x < leftboundry|| leftdown_x>rightboundry)
			break;
	}
        /*1-2之间直走*/
	cout<<"*********************"<<" 1-----2"<<"**********************************"<<endl;
	...
	motionProxy.moveToward(0.1,direction*0.8,0);
	if(direction==-1 && leftdown_x>rightboundry)
	{
		break;
	}
	if(direction==1 && rightdown_x>leftboundry)
	{
		break;
	}
 ...
 //        	waitKey(50);
	...
	motionProxy.moveToward(1, 0, 0, config);                 //   能否准确到达 终点
